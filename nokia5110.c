...
void nokia_lcd_clear_area(uint8_t x_st, uint8_t y_st, uint8_t x_end, uint8_t y_end){
	...
	for(x=x_st;x<=x_end;x++){
		for(y=y_st;y<=y_end;y++) nokia_lcd_set_pixel(x,y,0);
	}
}
void nokia_lcd_draw_bitmap(const uint8_t* img,uint8_t x,uint8_t y,uint8_t wdth,uint8_t hght){
     uint8_t x_pos,y_pos,y_co=1,x_cur=0;
	 for(int i=0;i<=(hght+hght%8)/8;i++){
		 	for(x_pos=0;x_pos<wdth;x_pos++,x_cur++){
			 	for(y_pos=0;y_pos<hght;y_pos++,y_co*=2){
				 	if(pgm_read_byte_near(img+x_cur)&y_co)nokia_lcd_set_pixel(x_pos+x,y_pos+y+(i*8),1);
					 else nokia_lcd_set_pixel(x_pos+x,y_pos+y+(i*8),0);
			 	}
			 	y_co=1;
		 	}
...
void nokia_lcd_inverse_area(uint8_t x_st, uint8_t y_st, uint8_t x_end, uint8_t y_end){
	...
	for(x=x_st;x<=x_end;x++){
		for(y=y_st;y<=y_end;y++){
            uint8_t *byte = &nokia_lcd.screen[y/8*84+x];
            if(*byte&(1<<(y %8 )))nokia_lcd_set_pixel(x,y,0);
            else nokia_lcd_set_pixel(x,y,1);
        } 
	}
}
